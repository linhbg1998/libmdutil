...
#define AMSVC_SYSTEM_PROFILER       CFSTR("com.apple.mobile.system_profiler")
  ...
  typedef unsigned int service_conn_t;
    ...
    */
    ...
    /*edits by greysyntax*/
unsigned int AMRecoveryModeDeviceSetAutoBoot(struct am_recovery_device *rdev, Byte paramByte);
    unsigned int AMRecoveryModeDeviceReboot(struct am_recovery_device *rdev, CFStringRef state);
    /*end*/
  ...
  int USBMuxConnectByPort(int connectionID, int iPhone_port_network_byte_order, int* outHandle);
    ...
    t_sendCommandToDevice sendCommandToDevice = (t_sendCommandToDevice)0x109199309;
